...
#ifdef USE_CUDNN
//#include "caffe/layers/cudnn_acl_conv_layer.hpp"
...
// Get convolution layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype>> GetACLConvLayer(
    const LayerParameter& param) {
  ACLConvParameter conv_param = param.convolution_param();
  ACLConvParameterParameter_Engine engine = conv_param.engine();
#ifdef USE_CUDNN
  bool use_dilation = false;
  for (int i = 0; i < conv_param.dilation_size(); ++i) {
    if (conv_param.dilation(i) > 1) {
...
#endif
  if (engine ==ACLConvParameter_Engine_DEFAULT) {
    engine = ACLConvParameter_Engine_CAFFE;
#ifdef USE_CUDNN
    if (!use_dilation) {
      engine = ACLConvParameter_Engine_CUDNN;
    }
#endif
  }
  if (engine == ACLConvParameter_Engine_CAFFE) {
    return shared_ptr<Layer<Dtype> >(new ACLConvLayer<Dtype>(param));
#ifdef USE_CUDNN
  } else if (engine ==ACLConvParameter_Engine_CUDNN) {
    if (use_dilation) {
      LOG(FATAL) << "CuDNN doesn't support the dilated convolution at Layer "
                 << param.name();
    }
    return shared_ptr<Layer<Dtype> >(new CuDNNACLConvLayer<Dtype>(param));
#endif
  } else {
    LOG(FATAL) << "Layer " << param.name() << " has unknown engine.";
    throw;  // Avoids missing return warning
...
REGISTER_LAYER_CREATOR(ACLConv, GetACLConvLayer);
